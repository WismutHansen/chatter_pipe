...
To run with specific options (e.g., file output):
  uv run chatter_daemon.py --output-mode file --output-dir ./my_audio_files &
...
- To send text to the daemon, write to this pipe. For example:
    ...
    echo "Another sentence for TTS." > /tmp/chatter_fifo Each line sent to the pipe is processed as a separate sentence.
...
def perform_cleanup():
    ...
    if _pipe_name_ref and os.path.exists(_pipe_name_ref):
        try:
            ...
            if _args_debug_ref:
                print(f"Removed named pipe: {_pipe_name_ref}", file=sys.stderr)
        except OSError as e:
            print(
                f"Error removing named pipe {_pipe_name_ref} during cleanup: {e}",
                file=sys.stderr,
            )
    print(
        "Cleanup complete.", file=sys.stderr
    )  # Changed message slightly to differentiate from signal-specific one
    ...
    while True:
        ...
        if output_mode == "play":
            ...
            try:
                ...
                filename_prefix = (
                    "".join(filter(str.isalnum, sentence_text[:30]))
                    .lower()
                    .replace(" ", "_")
                    or "speech"
                )
                if (
                    not filename_prefix
                ):  # Handle cases where sentence might be all non-alnum
                    filename_prefix = "speech"
                output_filename = os.path.join(
                    output_dir, f"{timestamp}_{filename_prefix}.wav"
                )

                sf.write(
                    output_filename, wav_tensor.squeeze(0).cpu().numpy(), sample_rate
                )
                print(f"Saved audio to: {output_filename}", file=sys.stderr)
            except NameError:  # If sf (soundfile) was not imported
                print(
                    "Error: The 'soundfile' library is required for file output but not installed. Please install it (e.g., pip install soundfile).",
                    file=sys.stderr,
                )
            except Exception as e:
                if output_filename:  # If filename was generated
                    print(
                        f"Error saving audio to file {output_filename}: {e}",
                        file=sys.stderr,
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Background TTS daemon using a named pipe."
    )
    parser.add_argument(
        "--pipe-name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to reference voice audio file to be used)",
    )
    parser.add_argument(
        "-e",
        "--exaggeration",
        type=float,
        default=0.5,
        help="Style exaggeration (default: 0.5)",
    )
    parser.add_argument(
        "-t",
        "--temperature",
    ...
    if args.debug:
        print("Loading ChatterboxTTS model...", file=sys.stderr)
    ...
    audio_thread = threading.Thread(
        target=audio_worker, args=(args.output_mode, args.output_dir), daemon=True
    )
    _audio_thread_ref = audio_thread  # Set global ref
    _audio_task_queue_ref = audio_task_queue  # Set global ref
    audio_thread.start()
    if args.debug:
        print(f"Audio worker thread started. Mode: {args.output_mode}", file=sys.stderr)
    ...
    if not os.path.exists(pipe_name):
        try:
            os.mkfifo(pipe_name)
            if args.debug:
                print(f"Created named pipe: {pipe_name}", file=sys.stderr)
        except OSError as e:
            print(f"Failed to create named pipe: {e}", file=sys.stderr)
            sys.exit(1)
    else:
        if args.debug:
            print(f"Using existing named pipe: {pipe_name}", file=sys.stderr)
    ...
    if args.debug:
        print("Registered signal handlers for SIGTERM and SIGINT.", file=sys.stderr)

    try:
        while True:
            if args.debug:
                print(f"Opening pipe {pipe_name} for reading...", file=sys.stderr)
            with open(pipe_name, "r") as fifo:
                if args.debug:
                    print(
                        f"Pipe {pipe_name} opened. Waiting for input...",
                        file=sys.stderr,
                    )
                while True:  # Keep reading from the currently open pipe
                    ...
                    if not line:  # Pipe closed by writer or empty line if writer closes and reopens
                        if args.debug:
                            print(
                                "Writer closed the pipe or sent empty line. Re-opening.",
                                file=sys.stderr,
                            )
                    ...
                    if args.debug:
                        print(f"Received from pipe: '{sentence}'", file=sys.stderr)
                    ...
                    wav_tensor = model.generate(
                        sentence,
                        audio_prompt_path=args.ref_voice,
                    ...
                    print(
                        f"Queued for {args.output_mode}: '{sentence}'", file=sys.stderr
                    )
        ...
        if args.debug:
            print(
                "Main loop/try block exited, entering finally for cleanup.",
                file=sys.stderr,
            )
        perform_cleanup()
